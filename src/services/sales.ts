...
export const salesService = {
  ...
  async getSalesByCustomer(
    ...
    // Filter by search query if provided (client-side filtering for product names)
    if (searchQuery && data) {
      const filteredData = data.filter(sale => {
        ...
        // Search in sale ID
        if (saleId.includes(query)) return true;
        
        // Search in product names
        if (sale.carts?.cart_items) {
          return sale.carts.cart_items.some(item => 
  ...
  async getCustomerSalesStats(businessId: string, customerId: string) {
    ...
    const totalSpent = data.reduce((sum, sale) => sum + sale.total_amount, 0);
    ...
    const thisMonthSales = data.filter(sale => 
      new Date(sale.sale_date) >= startOfMonth
    );
    const thisMonthSpent = thisMonthSales.reduce((sum, sale) => sum + sale.total_amount, 0);
