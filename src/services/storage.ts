...
export const storageService = {
  ...
  async uploadBusinessImage(file: File | { uri: string; type: string; name: string }, businessId?: string): Promise<UploadResult> {
    // Generate unique filename
    const timestamp = Date.now();
    const randomString = Math.random().toString(36).substring(2, 15);
    ...
    if (Platform.OS === 'web' && 'size' in file) {
      ...
      const fileExtension = webFile.name.split('.').pop()?.toLowerCase() || 'jpg';
      fileName = `business_${businessId || 'temp'}_${timestamp}_${randomString}.${fileExtension}`;
      contentType = webFile.type;
      ...
      try {
        ...
        // Get Supabase URL and project reference
        const supabaseUrl = supabase.supabaseUrl;
        const uploadUrl = `${supabaseUrl}/storage/v1/object/business-images/${filePath}`;
  ...
  async updateBusinessImage(oldImageUrl: string | null, newFile: File | { uri: string; type: string; name: string }, businessId: string): Promise<UploadResult> {
    // Upload new image first
    const uploadResult = await this.uploadBusinessImage(newFile, businessId);

    // Delete old image if it exists and is from our storage
    if (oldImageUrl && oldImageUrl.includes('business-images')) {
      try {
        await this.deleteBusinessImage(oldImageUrl);
      } catch (error) {
        console.warn('Failed to delete old image:', error);
        // Don't throw error here as the new image was uploaded successfully
      }
    ...
    try {
      ...
      if (imageUrl.includes('/product-images/')) {
        ...
        if (pathParts.length > 1) {
          return pathParts[1].split('?')[0]; // Remove query parameters
        }
      }
      ...
      if (imageUrl.includes('/profile-images/')) {
        ...
        // Fallback for different URL formats
        const pathParts = imageUrl.split('/profile-images/');
        if (pathParts.length > 1) {
          return pathParts[1].split('?')[0]; // Remove query parameters
        }
      }
      
      // Check for business images bucket
      if (imageUrl.includes('/business-images/')) {
        const urlParts = imageUrl.split('/storage/v1/object/public/business-images/');
        if (urlParts.length > 1) {
          return urlParts[1].split('?')[0]; // Remove query parameters
        }
        
        // Fallback for different URL formats
        const pathParts = imageUrl.split('/business-images/');
        if (pathParts.length > 1) {
          return pathParts[1].split('?')[0]; // Remove query parameters
        }
      }
  ...
  async uploadProfileImage(file: File | { uri: string; type: string; name: string }, userId: string): Promise<UploadResult> {
    // Generate unique filename
    const timestamp = Date.now();
    const randomString = Math.random().toString(36).substring(2, 15);
    ...
    if (Platform.OS === 'web' && 'size' in file) {
      ...
      const fileExtension = webFile.name.split('.').pop()?.toLowerCase() || 'jpg';
      fileName = `profile_${userId}_${timestamp}_${randomString}.${fileExtension}`;
      contentType = webFile.type;
  ...
  async updateProfileImage(oldImageUrl: string | null, newFile: File | { uri: string; type: string; name: string }, userId: string): Promise<UploadResult> {
    // Upload new image first
    const uploadResult = await this.uploadProfileImage(newFile, userId);
