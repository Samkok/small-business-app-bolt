...
import { processSalesImport, processSalesImportFromFile } from '@/src/utils/salesImportProcessor';
...
export default function ImportSalesModal({ onClose, onComplete }: ImportSalesModalProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (file) {
        results = await processSalesImportFromFile(file, profile.id);
      } else if (csvContent) {
        results = await processSalesImport(csvContent, profile.id);
      } else {
  ...
  return (
    <View style={[styles.container, { backgroundColor: isDark ? '#111827' : '#f9fafb' }]}>
      ...
      <View style={styles.footer}>
        ...
        />
        <Button
          title={loading ? 'Importing...' : 'Import Sales'}
