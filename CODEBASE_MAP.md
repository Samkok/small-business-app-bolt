...
**Reports**: `reportsService` (~1,600 lines, 18 methods incl. `getIncomeStatement`, shared by the screen and CSV) feeds dashboard (`(tabs)/index`), `reports/*`, `top-products`, `top-customers`. COGS comes from RPC `calculate_cogs(business, from, to, currency)`.
