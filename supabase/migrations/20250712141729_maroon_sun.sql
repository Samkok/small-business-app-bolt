/*
  # Create user_profiles and user_business_roles tables

  1. New Tables
    - `user_profiles`: Stores user-specific data independent of businesses
    - `user_business_roles`: Manages many-to-many relationships between users and businesses
    
  2. Indexes
    - Create indexes for efficient querying on user_id and business_id
    
  3. Security
    - Enable RLS on new tables
    - Create policies for user_profiles (users can manage their own profile)
    - Create policies for user_business_roles (users can read their roles, admins can manage roles)
*/
