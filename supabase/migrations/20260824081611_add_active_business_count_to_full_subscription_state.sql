/*
# Add activeBusinessCount to get_full_subscription_state

Returns the count of active (non-disabled) businesses alongside the total
so the client can correctly determine whether creation is allowed.
*/

CREATE OR REPLACE FUNCTION get_full_subscription_state(p_user_id uuid, p_business_id uuid DEFAULT NULL::uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Get total owned business count
  SELECT COUNT(*)::integer
  INTO v_owned_business_count
  FROM businesses
  WHERE owner_user_id = p_user_id;

  -- Get active business count (only active state, not owner_disabled or read_only_sales)
  SELECT COUNT(*)::integer
  INTO v_active_business_count
  FROM businesses
  WHERE owner_user_id = p_user_id
