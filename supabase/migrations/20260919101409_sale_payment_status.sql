/*
  # Payment status on a sale: PAID or COD (cash on delivery)

  ## sales.payment_status
  - 'paid' : the customer has already paid
  - 'cod'  : cash on delivery, the money is collected when the parcel arrives
  - NULL   : sales made before this field existed. Nothing is known about them, so
             no historical row is rewritten; they can be set from the sale's edit form.

  ## complete_sale_atomic
  Gains one trailing parameter, p_payment_status text DEFAULT NULL, written to the
  new column in the same transaction as the sale. The body is otherwise identical to
  the live function (taken from pg_get_functiondef on 2026-09-19).

  Adding a parameter changes the signature, and CREATE OR REPLACE would then create a
  SECOND overload next to the 15-argument one, making every named-argument call
  ambiguous and breaking checkout. So the old signature is dropped and the new one
  created in this single transaction, leaving exactly one function. The new parameter
  is optional, so app versions already installed (which do not send it) keep working.
  Grants are restored to what the function had before.
*/
...
ALTER TABLE public.sales
  ...
  CHECK (payment_status IS NULL OR payment_status = ANY (ARRAY['paid'::text, 'cod'::text]));

CREATE INDEX IF NOT EXISTS idx_sales_business_payment_status
  ON public.sales (business_id, payment_status)
  WHERE payment_status IS NOT NULL;

DROP FUNCTION IF EXISTS public.complete_sale_atomic(
  uuid, uuid, uuid, numeric, text, timestamp with time zone, text, uuid, text, numeric, numeric, numeric, numeric, uuid, numeric
);

CREATE FUNCTION public.complete_sale_atomic(
  p_cart_id uuid,
  p_customer_id uuid,
  p_business_id uuid,
...
GRANT EXECUTE ON FUNCTION public.complete_sale_atomic(
  uuid, uuid, uuid, numeric, text, timestamp with time zone, text, uuid, text, numeric, numeric, numeric, numeric, uuid, numeric, text
