/*
  # Fix get_full_subscription_state Function

  1. Changes
    - Fix table reference from `business_sales_counts` to `user_sales_counts`
    - Update query to use correct composite key (user_id, business_id)
    - Fix sales count calculation to use actual table structure
    
  2. Impact
    - Makes the function work correctly
    - Fixes the optimized subscription state query
*/
...
BEGIN
  ...
  IF p_business_id IS NOT NULL THEN
    ...
    -- Get sales count data from user_sales_counts table
    SELECT 
    ...
    IF v_tier = 'free' THEN
      v_remaining_sales := GREATEST(0, 50 - v_sales_count);
      v_is_at_limit := v_sales_count >= 50;
    ELSE
